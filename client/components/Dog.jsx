...
import {
    Button, ProgressBar, Card, Dropdown, DropdownButton, Modal, Form,
...
function Dog(props) {
    ...
    const getDog = () => {
        axios.get(`/dog/id/${dog._id}`).then(({data}) => setDog(data)).catch((err) => {
            console.error(err);
        });
    };

    const getSignedInUserMeals = (userIdParam) => {
        axios.get(`/user/meals/${userIdParam}`).then(({data}) => {
    ...
    const getSignedInUserMedicines = (userIdParam) => {
        axios.get(`/user/medicines/${userIdParam}`).then(({data}) => {
    ...
    const feedDog = (dogToFeedObj, mealToFeedObj) => {
        ...
        axios.put(`dog/${dogToFeedObj._id}`, {status}).then(getDog()).then(() => {
            axios.put(`user/meals/${user._id}`, {
                update: {
                    type: 'deleteMeal',
    ...
    const giveMedicine = (dogToGiveMeds, medsToGiveObj) => {
            const status = {
                medicineDeadline: new Date(new Date(dogToGiveMeds.medicineDeadline).getTime() + 24 * 60 * 60 * 1000,),
            };

            axios.put(`dog/${dogToGiveMeds._id}`, {status}).then(getDog()).then(() => {
                axios.put(`user/medicines/${user._id}`, {
                    update: {
                        type: 'deleteMedicine',
    ...
    const handleClick = (e) => {
        ...
        if (e === 'walk') {
            ...
            axios.put(`/dog/${dog._id}`, {status}).then(() => getDog()).catch((err) => {
                console.error(err);
            });
            ...
            axios.put(`/dog/${dog._id}`, {status, cost: -3}).then(({data}) => {
                setCoins(data.coinCount);
            }).then(() => getDog()).catch((err) => {
                console.error(err);
            });
    ...
    const fetchAndShowWord = () => {
        ...
        axios.get('/words/randomWord').then(({data}) => {
    ...
    const addWordToDogtionary = () => {
        ...
        axios.post('/words/dogtionary', {
            wordObj: word,
        }).then(() => setAdded(true)).catch((err) => {
    ...
    const openDogtionary = () => {
        ...
        axios.get(`/words/dog/${dog._id}`).then(({data}) => {
            setDogtionary(data);
    ...
    return (<div>
        {groomed ? (<Card className='d-flex flex-row m-4 card text-white bg-warning mb-3'>
            <div
        ...
        </Card>) : (<Card className='d-flex flex-row m-4'>
            <div
                className='d-flex flex-column justify-content-center align-items-center align-self-center '
                style={{width: '250px', height: '250px'}}
            >
            ...
            <div className='d-flex flex-column justify-content-center align-items-center w-100 '>
                ...
                <Card.Body className='w-100'>
                    <div className='dog-status'>
                        ...
                        </DropdownButton>) : (<DropdownButton title='Feed from Pantry!'>
                            <Dropdown.Item>
                                Visit Bone Appétit Café to buy your first meal!
                            </Dropdown.Item>
                        </DropdownButton>)}
                        {medicines ? (<DropdownButton title='Cure with Meds!'>
                            {medicines.map((medicine) => (<Dropdown.Item
                                key={medicine._id}
                                onClick={() => {
