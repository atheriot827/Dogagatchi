...
function PoochBattles({ show, onHide, playerDog, enemyDog, onBattleEnd }) {
  ...
  const [battleState, setBattleState] = useState({
    playerHealth: playerDog.health, // Players dog starting health
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setBattleState((prev) => ({
          ...
          battleLog: [
            ...prev.battleLog,
            `${enemyDog.name} used ${randomMove.name}!`,
          ],
          currentTurn: 'player',
        }));
  ...
  return (
    ...
    >
      <Modal.Header className='bg-dark text-light'>
        <Modal.Title>
          Battle vs {enemyDog?.name || 'Unknown Enemy'}!
        </Modal.Title>
      </Modal.Header>
      ...
      <Modal.Body className='bg-secondary'>
        <div className='battle-arena d-flex justify-content-between align-items-center p-3'>
          ...
          <div className='player-dog text-center'>
            <img
              src={`/assets/gifs/${playerDog?.breed || 'doberman'}/${
        ...
        <div className='battle-controls d-flex justify-content-center gap-3 my-3'>
          {Object.keys(moves).map((move) => (
            <Button
              ...
              disabled={
                !battleState.isActive || battleState.currentTurn !== 'player'
              }
              variant={
                battleState.currentTurn === 'player' ? 'primary' : 'secondary'
              }
        ...
        <div className='battle-log bg-dark text-light p-3 rounded'>
          {battleState.battleLog.slice(-3).map((log, index) => (
            <p
              key={index}
              className={`mb-1 ${
                log.includes(playerDog.name) ? 'text-primary' : 'text-danger'
              }`}
            >
              {log}
            </p>
