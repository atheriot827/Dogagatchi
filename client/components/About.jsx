...
const mappedTechStack = techStack.map((tech, index) => (
  <Card key={index} style={{ margin: '10px', width: '200px' }}>
    <div style={{
      display: 'flex', width: '150px', height: '150px', alignItems: 'center', justifyContent: 'center',
    }}
    >
    ...
    <Card.Body
      ...
      style={{
        backgroundImage: 'linear-gradient(#182950, #274282)', color: 'white', width: '175px', borderRadius: '5px',
      }}
    >
...
const mappedMongeese = mongeese.map((mongoose, index) => (
  <Card
    key={index}
    style={{
      width: '230px', margin: '10px', backgroundImage: 'linear-gradient(#182950, #274282)', border: 'white solid 2px',
    }}
  >
    <Card.Body className="text-center p-3" style={{ color: 'white' }}>

      <Card.Img src={mongoose.src} />
