        ...
        Object.keys(newPositions).forEach(dogId => {
          if (dogId !== currentDog?._id) {
            ...
            if (Math.random() < 0.01) {
              ...
              position.action = actions[Math.floor(Math.random() * actions.length)];
            }
          }
        });
          ...
          dog._id !== currentDog?._id && (
            <div
              ...
              style={{
                ...
                transform: `scaleX(${dogPositions[dog._id]?.direction === 'left' ? -1 : 1})`,
                transition: 'all 0.1s ease-in-out',
                zIndex: 1
              }}
            >
              <img
                src={`/assets/gifs/${dog.breed}/${dogPositions[dog._id]?.action ?? 'Standing'}.gif`}
                alt={dog.name}
